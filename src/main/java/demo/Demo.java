...
public class Demo {
    ...
    public static ImporterExporterJson iEJson = new ImporterExporterJson();
    public static IRoot root = new Root();
    public static ILocal local = new Local(0, "", 0, null);
    public static IPlayer player = new Player("", "", 0, 0, 0, 0, 0, null);
    public static String playerName = "";

    /**
     * Instância do exportar que transforma o grafo em uma imagem
     */
    public static final IExporter exporter = new ExporterGraph("docs/export");
    ...
    private static void runGameFirst() throws IOException, EmptyCollectionException, NotLocalInstanceException, java.text.ParseException {
        ...
        switch (option) {
            ...
            case 3:
                IConnector connectorCooldownGoPortalRandom = root.getRandomConnector(); //obter o connector sem cooldown mais perto e depois portal

                showConnectorCooldownGoPortalMenu(connectorCooldownGoPortalRandom, firstTime);
                break;

            case 4:
                IConnector connectorCooldownGoConnectorRandom = root.getRandomConnector(); //obter o connector sem cooldown mais perto e depois connector
    ...
    private static void runGameSecond(ILocal local) throws EmptyCollectionException, NotLocalInstanceException, java.text.ParseException, IOException {
        ...
        switch (option) {
            case 1:
                IPortal portalRandom = (IPortal) root.getRoute(root, 1, local).next(); //obter o portal mais perto
                String teamPlayer = root.getPlayerByName(playerName).getTeam();//obter equipa do jogador atual
                String ownershipPortal = portalRandom.getOwnership().getState(); //obter o dono do portal
            ...
            case 2:
                IConnector connectorRandom = (IConnector) root.getRoute(root, 2, local).next(); //obter o connector mais perto

                showConnectorMenu(connectorRandom, firstTime);
                break;

            case 3:
                IConnector connectorCooldownGoPortalRandom = (IConnector) root.getRoute(root, 3, local).next(); //obter o connector sem cooldown mais perto e depois portal

                showConnectorCooldownGoPortalMenu(connectorCooldownGoPortalRandom, firstTime);
                break;

            case 4:
                IConnector connectorCooldownGoConnectorRandom = (IConnector) root.getRoute(root, 4, local).next(); //obter o connector sem cooldown mais perto e depois connector
    ...
    private static void showConnectorCooldownGoPortalMenu(IConnector connectorRandom, boolean firstTime) throws EmptyCollectionException, NotLocalInstanceException, java.text.ParseException, IOException {
        Scanner scanner = new Scanner(System.in);
        boolean exit = false;
        int option = 0;
        IInteraction interaction = null;
        ...
        if (firstTime == false) {
            while (!exit) {
                ...
                System.out.println("you are on the connector nº " + connectorRandom.getId());
                System.out.println("+--------------------------------------+");
                System.out.println("| CONNECTOR W/O COOLDOWN INTERACTIONS  |");
                System.out.println("+--------------------------------------+");
                System.out.println("your energy: " + energyCurrentPlayer);
                System.out.println("your max energy: " + energyMaxPlayer);
                System.out.println("energy connector: " + energyConnector);
                ...
                switch (option) {
                    case 1:
                        String date = String.valueOf(root.getConnectorInteractionsByPlayerName(connectorRandom.getId(), playerName).getDate());//data da ultima interação do jogador com connector
                        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss");
                        LocalDateTime dateLastInteraction;
